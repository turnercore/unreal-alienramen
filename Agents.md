...
- local-only HUD prediction helpers exist on PlayerState (`SetPredictedSpiceValue`, `ClearPredictedSpiceValue`) and never mutate authoritative spice values; they now optimistically broadcast the normal `OnSpiceChanged` delegate locally, and authoritative replication/attribute updates emit `OnSpiceChanged` again only if correction is needed.
...
- local prediction helpers for cursor exist on PlayerState (`SetPredictedSpicyTrackCursorTier`, `ClearPredictedSpicyTrackCursorTier`) and never mutate authoritative cursor state; they now optimistically broadcast the normal `OnSpicyTrackCursorChanged` delegate locally, and `OnRep_SpicyTrackCursorTier` emits correction only when authoritative cursor differs from predicted.
