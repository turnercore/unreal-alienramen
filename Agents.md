...
- Gameplay-mode character pawn/runtime repair now lives in opt-in `AARGameModeBase` bootstrap hooks. Shop, Invader, and Scrapyard should reuse that shared sequence for `BeginPlay`, restart/join repair, and seamless-travel repair instead of reintroducing duplicated local orchestration. Lobby and Transition should stay off that path unless explicitly redesigned.
- `UARUnlockReactiveComponent` must not evaluate non-empty `RequiredUnlockTags` against a pre-hydration empty unlock container. Use `AARGameStateBase::HasHydratedFromSave()` / `OnHydratedFromSave` to gate startup evaluation so placed actors do not false-lock before default/save unlocks are applied.
