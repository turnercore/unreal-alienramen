    ...
    def run_validation(self, value):
        ...
        if not isinstance(value, Sequence):
            raise ValidationError(
                "Expected a sequence of mappings, but a %s was given." % type(value)
            )
        ...
        for item in value:
            if len(item) != 1:
                raise ValidationError(
                    "Expected a sequence of mappings with one item each, got {0}".format(
                        len(item)
                    )
                )
            result.append(
                dict((key, self.item_config.validate(entry)) for key, entry in item.items())
            )
        return result
