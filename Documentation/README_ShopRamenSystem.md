...
- Shop startup order should remain: hydrate/repair character runtime data, materialize both canonical shop pawns, bind runtime/pawn/player-state state, then perform final possession routing for active players.
...
- Shop now opts into the shared `AARGameModeBase` gameplay bootstrap helper for `BeginPlay`, restart repair, and seamless-travel repair.
- Shop-specific bootstrap work that stays mode-owned: save/economy deposit, transient carryable restore, energy-drink anchor spawning, save-driven transform/held-item restore, and canonical shop-entry persistence.
